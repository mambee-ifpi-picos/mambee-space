...
function ListaSalasPage() {
  ...
  useEffect(() => {
    const verificarAdmin = async () => {
      ...
      if (session?.user) {
        try {
          const res = await fetch(`/api/usuarios/me?idAuth=${session.user.id}`);
          if (res.ok) {
  ...
  const carregarSalas = useCallback(async (searchTerm?: string) => {
    ...
    try {
      ...
      if (!res.ok || !json.success) throw new Error(json.error || "Erro ao carregar salas.");
      setSalas(json.salas || []);
    } catch (e: unknown) {
                    ...
                    <tbody>
                      {loading ? (
                        ...
                        salasFiltradas.map((sala, index) => (
                          <tr key={sala.idSala} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                            <td className="px-4 py-3 align-middle whitespace-nowrap">{sala.idSala}</td>
                            <td className="px-4 py-3 align-middle min-w-[180px] sm:min-w-[220px]">
                              <div className="truncate max-w-[180px] sm:max-w-[220px]">{sala.nomeSala}</div>
                            </td>
                            ...
                            <td className="px-4 py-3 align-middle whitespace-nowrap">
                              <span
                                className={`text-xs font-medium ${
                                  sala.ativa === false ? "text-red-500" : "text-teal-500"
                                }`}
                              >
            ...
            {showSuccess && (
              <div className="fixed left-1/2 bottom-6 md:bottom-10 -translate-x-1/2 w-[min(480px,90%)]">
                <div className="flex items-center justify-between bg-teal-500 text-white rounded-md px-4 md:px-6 py-3 text-sm shadow-lg">
                  ...
                  <button type="button" onClick={() => setShowSuccess(false)} className="text-lg leading-none px-1">
                    ×
                  </button>
                </div>
              </div>
