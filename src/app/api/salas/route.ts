...
export const GET = Auth(async () => {
  try {
    ...
    if (error) {
      return NextResponse.json({ success: false, error: "Erro ao buscar salas." }, { status: 400 });
    }
...
export const POST = Auth(async (req: Request) => {
  try {
    ...
    if (limiteHorasReserva > 4) {
      return NextResponse.json({ success: false, error: "Limite máximo é 4 horas." }, { status: 400 });
    }
