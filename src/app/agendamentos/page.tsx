    ...
    <div className="min-h-screen bg-white p-4 md:p-8 font-sans text-gray-700">
      ...
      <div className="w-full">
        {!carregando && dados.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-200 text-center">
            <h3 className="text-xl font-bold text-gray-700 mb-2">Sem agendamentos disponíveis para hoje</h3>
          </div>
        ) : (
          <>
            <div className="block md:hidden space-y-4 pb-10">
              {dados.map((item, index) => {
                ...
                const avatarSrc =
                  item.Usuario?.foto && item.Usuario?.foto.length > 5
                    ? item.Usuario?.foto
                  ...
                  {dados.map((item, index) => {
                    ...
                    return (
                      <tr key={item.idReserva || index} className="group transition-transform hover:scale-[1.01]">
                        ...
                        <td className="bg-gray-50 p-4 text-sm text-gray-600">{item.Espaco?.Sala?.nomeSala || "—"}</td>
