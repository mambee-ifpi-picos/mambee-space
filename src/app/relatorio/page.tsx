  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-8 font-serif text-gray-800">
      <div className="mb-6 md:mb-8">
        <h1 className="text-3xl font-bold text-black uppercase tracking-wide text-center md:text-left">
          Relatório
        </h1>
      </div>

      {erro && (
        <div className="mb-6 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 text-center rounded">
          {erro}
        </div>
      )}
      ...
      <div className="mb-8 p-6 bg-gray-50 rounded-lg border border-gray-100 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
          <div>
            ...
            <input value={usuario} onChange={(e) => setUsuario(e.target.value)} className="w-full bg-white border border-gray-300 rounded p-3 focus:outline-none focus:ring-2 focus:ring-[#3BB1A8]" />
          </div>
          <div>
            <label className="block text-sm font-bold mb-1 text-gray-600">Sala</label>
            <input value={sala} onChange={(e) => setSala(e.target.value)} className="w-full bg-white border border-gray-300 rounded p-3 focus:outline-none focus:ring-2 focus:ring-[#3BB1A8]" />
          </div>
          <div>
            <label className="block text-sm font-bold mb-1 text-gray-600">Espaço</label>
            <input value={espaco} onChange={(e) => setEspaco(e.target.value)} className="w-full bg-white border border-gray-300 rounded p-3 focus:outline-none focus:ring-2 focus:ring-[#3BB1A8]" />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          <div>
            <label className="block text-sm font-bold mb-1 text-gray-600">Início</label>
            <input type="date" value={dataInicio} onChange={(e) => setDataInicio(e.target.value)} className="w-full bg-white border border-gray-300 rounded p-3 uppercase text-sm focus:outline-none focus:ring-2 focus:ring-[#3BB1A8]" />
          </div>
          <div>
            <label className="block text-sm font-bold mb-1 text-gray-600">Fim</label>
            <input type="date" value={dataFim} onChange={(e) => setDataFim(e.target.value)} className="w-full bg-white border border-gray-300 rounded p-3 uppercase text-sm focus:outline-none focus:ring-2 focus:ring-[#3BB1A8]" />
          </div>
          <div className="hidden md:block"></div>
          <button onClick={buscarRelatorio} disabled={carregando} className="w-full bg-[#3BB1A8] hover:bg-teal-600 text-white p-3 rounded font-bold uppercase transition flex items-center justify-center gap-2 shadow-sm">
      ...
      <div className="block md:hidden space-y-4 pb-10">
        {dados.map((item, index) => {
           ...
           const avatarSrc = item.foto && item.foto.length > 5 ? item.foto : `https://ui-avatars.com/api/?name=${encodeURIComponent(nomeExibido)}&background=0d9488&color=fff`;
           
           return (
             <div key={item.id || index} className="relative bg-white rounded-xl p-4 pl-6 shadow-sm border border-gray-200 flex items-center gap-4">
               ...
               <div className="flex-1 overflow-hidden">
                 ...
                 <span className="inline-block bg-teal-50 text-teal-700 text-[10px] font-bold px-2 py-0.5 rounded my-1 border border-teal-100">{item.espaco}</span>
                 <p className="text-xs text-gray-500 font-medium">{formatarData(item.inicio)} • {formatarHora(item.inicio)} - {formatarHora(item.fim)}</p>
               </div>
             </div>
           );
        })}
          ...
          <table className="min-w-full bg-white">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Horário</th>
